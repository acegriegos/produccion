...
function addline(idprod,cod,desc,cant,prec,tot,cntinv,dcs,mdcs,hinv,defi,uni,comodin,desgloce) {
    ...
    if (!existe) {
        ...
        $("#fd"+id).data('triforce',{vaccion : 0,vid : 0,vidfactura : '?',videntrada : idprod,vcantidad : cant,vprecio : precio,vdesc : dcs,vtotal : 0,vidinventario : hinv,vidodt : 0,vimv : 0,vcomodin : comodin,vidunidad : uni,vidimpuestos : '',viddescuentos : ''});
...
function totalizar(){
    ...
    $(".totp").each(function(){
        ...
        $(".dimpuesto").each(function(){
            ...
            rimv = parseFloat($(this).data('valores')['vmonto'])*parseFloat((1-(eimv/100))).toFixed(2);
...
function cargarProducto(kbrota,elemento) {
    ...
    if (cod[0][0] != undefined) {
        ...
        cargarImpuestos(cod[0].substr(1),tabla);
