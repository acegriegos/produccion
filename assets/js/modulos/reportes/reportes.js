    ...
    if (mdate != undefined) {
        if ($(".principal .filtros").attr('portipo') == 'varios') {
            ...
            for (var i = 0, len = vtbl.length; i < len; i++) {
                inc += 1;
                html = '<div class="row col s12 m6 l6 rous"><div class="col s3"><input type="checkbox" id="xtip'+inc+'" name="filtros" value="'+filtro+'" class="repcheck" elemlength="1" order="'+filtro+'"><label for="xtip'+inc+'" class="pbtn">'+tipos[i]+'</label></div><div class="col s9 '+mdate+'" id="fltr'+filtro+'"><div class="input-field"><select type="select" id="htip'+inc+'" class="inpreport tipos" ttbl="168"></select></div></div></div>';
                $(".principal .filtros").append(html);
                arr('login',6,'id,nombre',vtbl[i],'id > 0 order by id',15,1,$("#htip"+inc));
                filtro += 1;
            }
...
$(document).on("click",".fa-level-up",function(){
    $(".filtros").hide();
    $(".fa-level-down").show();
});

$(document).on("click",".fa-level-down",function(){
    $(".filtros").show();
    $(this).hide();
});
