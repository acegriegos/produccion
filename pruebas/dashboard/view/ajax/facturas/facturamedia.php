...
<?php 
  ...
  if($datos[24] == 3){
    ...
    $kakaroto->sql = 'select if(b.idinventario = 7,"Ruta","Inventario") as destinoa,ifnull(d.nombre,c.comodin),b.cantidad from msdetallefacturas a join detallefacturas b on b.id = a.viddetalle join facturas c on c.id = b.idfactura left join clientes d on d.id = c.idcliente where a.oc = '.$_GET['id'];
    ...
    if(sizeof($pedidos)){
        ...
        $str_ped = '<b>Pedidos</b><table id="lped" style="width:50%"> <tr>';
        for($i = 0; $i < sizeof($pedidos); $i++){
            
            $str_ped .= '<td>'.$pedidos[$i][0].'</td>
                         <td>'.$pedidos[$i][1].'</td>
                         <td>'.$pedidos[$i][2].'</td>';            
        }
        $str_ped .= '</tr> </table>';
